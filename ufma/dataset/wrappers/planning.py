...
from ufma.constants import PLANNER_PROMPT
...
class PlanningDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if route == 1:
            ...
            response = f'[{{"type": "grounder", "value": "{query}"}}, {{"type": "checker"}}, {{"type": "answerer"}}]'
        elif route == 2:
            # grounding + answering
            response = f'[{{"type": "grounder", "value": "{question}"}}, {{"type": "checker"}}, {{"type": "answerer"}}]'
        elif route == 3:
            # rephrasing + grounding
            response = f'[{{"type": "grounder", "value": "{query}"}}, {{"type": "checker"}}]'
        elif route == 4:
            # answering
